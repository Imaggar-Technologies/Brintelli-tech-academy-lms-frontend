...
const HrShortlistsList = () => {
  ...
  const loadShortlists = async () => {
    try {
      setLoading(true);
      const res = await jobsAPI.getShortlists({ page, limit: PAGE_SIZE });
      if (res.success && res.data?.shortlists) {
        setShortlists(res.data.shortlists);
        setTotal(res.data.total ?? 0);
      } else setShortlists([]);
    } catch (e) {
      toast.error(e.message || 'Failed to load shortlists');
      setShortlists([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleRemoveShortlist = async (applicationId) => {
    try {
      await jobsAPI.shortlistApplication(applicationId, false);
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-textMuted mt-1 text-sm">
          Candidates you have shortlisted from the Candidates page. Remove from shortlist to move them back to candidates only.
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-brintelli-border bg-brintelli-card p-12 text-center">
          <Star className="h-12 w-12 text-textMuted mx-auto mb-4" />
          <p className="text-textMuted">No shortlisted candidates yet.</p>
          <Button variant="primary" className="mt-4" onClick={() => navigate('/hr/candidates')}>
            Go to Candidates
          </Button>
        </div>
      ) : (
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  {c.shortlistedAt && (
                    <span className="text-xs text-textMuted">
                      Shortlisted {new Date(c.shortlistedAt).toLocaleDateString()}
                    </span>
                  )}
                  <Button variant="ghost" size="sm" onClick={() => navigate(`/hr/jobs/${c.jobId}`)}>
                    View job
                  </Button>
                  <Button variant="secondary" size="sm" onClick={() => handleRemoveShortlist(c.id)}>
...
export default HrShortlistsList;
