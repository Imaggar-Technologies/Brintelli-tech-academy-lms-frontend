...
const empty = { title: "", slug: "", description: "", screens: "", create_at: "", author: "", authorTitle: "", thumb: "", link: "" };
...
export default function CmsEventsList() {
  ...
  const load = async () => {
    try {
      const data = await getEntity("events");
  ...
  const save = async (e) => {
    ...
    try {
      ...
      if (editing) await putEntity("events", editing.id, payload);
      else await postEntity("events", payload);
      setEditing(null);
      setForm({ ...empty });
      load();
    } catch (e) {
  ...
  const remove = async (item) => {
    ...
    try {
      await deleteEntity("events", item.id);
      load();
    } catch (e) {
      setError(e.message);
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className={card}>
        <h3 className="mb-4 text-lg font-medium">{editing ? "Edit" : "Add"} Event</h3>
