...
const BatchSessions = () => {
  ...
  const addMaterial = () => {
    if (newMaterial.title && newMaterial.url) {
      ...
      toast.error('Please provide both title and URL for the material');
    }
  };
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <select
                  name="objectiveIds"
                  multiple
                  value={formData.objectiveIds || []}
                  onChange={(e) => {
                    const selected = Array.from(e.target.selectedOptions, option => option.value);
                    setFormData({ ...formData, objectiveIds: selected });
                  }}
                  className="w-full px-4 py-2 border border-brintelli-border rounded-lg bg-brintelli-card text-text min-h-[120px]"
                  required
                  size={Math.min(availableObjectives.length + 1, 6)}
                >
                ...
                <p className="text-xs text-textMuted mt-1">
                  Hold Ctrl (Windows) or Cmd (Mac) to select multiple objectives. {formData.objectiveIds?.length || 0} selected.
                </p>
              </div>
