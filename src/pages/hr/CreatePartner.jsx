...
const CreatePartner = () => {
  ...
  return (
    <div className="max-w-2xl">
      <h1 className="text-2xl font-bold text-text mb-6">Add partner company or college</h1>
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <textarea name="notes" value={form.notes} onChange={handleChange} rows={3} className="w-full rounded-lg border border-brintelli-border bg-brintelli-baseAlt px-3 py-2 text-text" />
        </div>
        <div className="flex gap-3 pt-4">
          <Button type="submit" variant="primary" disabled={saving}>{saving ? 'Saving...' : 'Save partner'}</Button>
          <Button type="button" variant="ghost" onClick={() => navigate('/hr/partners')}>Cancel</Button>
        </div>
      </form>
    </div>
  );
